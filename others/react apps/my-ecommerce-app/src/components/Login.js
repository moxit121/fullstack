...
const SignIn = () => {
  ...
  const submitHandler = (e) => {
    ...
    if (email !== "") {
      ...
      const userEmail = userData.filter((item) => {
        return item.email === email;
      });
      if (userEmail.length !== 0) {
        if (password !== "") {
          const userfound = userEmail.filter((item) => {
  ...
  return (
    <section className="container-fluid h-screen flex items-center justify-center mt-14">
      <form
        className="w-[500px] h-fit flex flex-col items-center md:w-1/2"
      ...
      >
        ...
        <input
          className="w-11/12 h-10 mb-5 ps-5 border border-gray-400 rounded-md input"
          type="email"
          onChange={(e) => {
